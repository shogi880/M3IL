...
flags.DEFINE_string(
    'datasetdir', os.path.join(os.path.dirname(os.path.realpath(__file__)), '../../datasets'),
...
flags.DEFINE_string(
    'reparam_embedding_type', "None", 'choose from [None, kl_2, include_kl, include_kl_2]')
flags.DEFINE_string(
...
def main():
    ...
    if FLAGS.load:
        checkpointdir = os.path.join(FLAGS.checkpoint_dir, FLAGS.experiment_name, 'model')
        loader = Loader(savedir=checkpointdir,
                        checkpoint=FLAGS.checkpoint_iter)
    p = Pipeline(consumers, saver=saver, loader=loader, learning_rate=FLAGS.lr, grad_clip=FLAGS.grad_clip)
    train_outs = p.get_outputs()
    # print('========================')
    summary_w = None
    if FLAGS.eval_only:
        log_dir = os.path.join(FLAGS.logdir, FLAGS.experiment_name, "eval",
        str(FLAGS.image_type) + "_sentence_type_" + str(FLAGS.sentence_type),
        "random_seed_" + str(FLAGS.random_seed) + "_" + datetime.datetime.now().strftime('%Y-%m-%d-%H-%M'))
    else:
        log_dir = os.path.join(FLAGS.logdir, FLAGS.experiment_name, "sentence_type_" + str(FLAGS.sentence_type))
    print(f"""
          {'*' * 70} 
    ...
    if FLAGS.eval:
        ...
        econs = EvalConsumer(data, sequencer, FLAGS.support, disk_images, FLAGS.sentence)
        if FLAGS.loss_type == 'margin_loss':
            task_emb = TaskEmbedding(network=net,
                                     embedding_size=FLAGS.embedding,
                                     include_state=False,
